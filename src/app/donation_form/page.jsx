...
const [user2, setUser2] = useState({});
  ...
  const charityTypes = [
    ...
    {
      value: "sadaqah_nafilah",
      label: "Sadaqah Nafilah",
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast.success(
        <div className="flex flex-col">
          ...
          <span className="text-sm">
            Receipt: {result.data.donation.receiptNumber}
          </span>
          <span className="text-sm">
            PKR {data.amount} for{" "}
            {charityCategories.find((c) => c.value === data.category)?.label}
          </span>
        </div>,
        { duration: 5000 }
      );
      ...
      if (result.data.notifications) {
        const { emailSent, smsSent, emailError, smsError } =
          result.data.notifications;

        if (emailSent || smsSent) {
          const notificationMessage = [];
          if (emailSent) notificationMessage.push("📧 Email sent");
          if (smsSent) notificationMessage.push("📱 SMS sent");
          ...
          if (notificationMessage.length > 0) {
            toast.success(
              <div className="flex flex-col">
                ...
                <span className="text-sm">
                  {notificationMessage.join(" & ")} to donor
                </span>
              </div>,
              { duration: 4000 }
            );
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Donor will receive confirmation email
                      </p>
                    </div>
                  </div>
                  <div
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Donor will receive thank you SMS
                      </p>
                    </div>
                  </div>
                  <div
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-6">
            <button
              ...
              onClick={() => {
                if (
                  window.confirm("Are you sure? All entered data will be lost.")
                ) {
                  reset();
                  setSendEmail(true);
                  setSendSMS(true);
